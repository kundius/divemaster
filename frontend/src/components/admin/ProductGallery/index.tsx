...
//   const url = `product/${productId}/images`
...
//     setLoading(true)
//     try {
//       const data = await apiGet<VespProductFile[]>(url, {}, withToken(auth.token)())
//       setItems(
//         data.rows.map((item) => ({
//           ...item,
//           id: item.file_id
...
import { useAuth } from '@/lib/auth/use-auth'
import { getImageLink } from '@/lib/utils'
import { VespFile, VespProductFile } from '@/types'
import {
  DndContext,
  DragEndEvent,
  KeyboardSensor,
...
interface SortableItemProps {
  ...
  width: number
  height: number
  setItems: Dispatch<SetStateAction<VespFile[]>>
}
...
export function ProductGallery(props: ProductGalleryProps) {
  ...
  const [loading, setLoading] = useState(false)
  const [sorted, setSorted] = useState(false)
  const [items, setItems] = useState<VespFile[]>([])
  const sensors = useSensors(
    useSensor(PointerSensor),
    useSensor(KeyboardSensor, {
      coordinateGetter: sortableKeyboardCoordinates
  ...
  async function onAddFiles(files?: FileList) {
    ...
    const asyncLoad = async (file: File): Promise<VespFile> => {
      ...
      //   reader.onload = async ({ target }) => {
      //     if (target?.result) {
      //       // const data = await apiPost<VespFile>(url, {
      //       //   file: target.result,
      //       //   metadata: { name: file.name, size: file.size, type: file.type }
      //       // })
      //       const body = new FormData()
      //       body.append('file', target.result as unknown as Blob)
      //       // body.append('name', file.name)
      //       // body.append('size', String(file.size))
      //       // body.append('type', file.type)
      //       console.log(body, target.result)
      //       const data = await api<VespFile>(url, {
      //         method: 'POST',
      //         body
      //       })
      //       resolve(data)
  ...
  async function fetch() {
    setLoading(true)
    try {
      const data = await apiGet<VespFile[]>(url, {}, withToken(auth.token)())
      setItems(data)
    } finally {
      setLoading(false)
    }
